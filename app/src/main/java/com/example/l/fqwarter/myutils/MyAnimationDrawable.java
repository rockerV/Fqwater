...
public class MyAnimationDrawable {
        ...
        loadRaw(resourceId, imageView.getContext(),
                new OnDrawableLoadedListener() {
                    ...
                    public void onDrawableLoaded(List<MyFrame> myFrames) {
                        ...
                        animateRawManuallyRoll(myFrames, imageView, onComplete);
                    }
                });
    }
    ...
    //
    private static void animateRawManuallyRoll(List<MyFrame> myFrames,
                                           ImageView imageView, Runnable onComplete) {
        animateRawManuallyRoll(myFrames, imageView, onComplete, 0, true);
    }

    //
    private static void animateRawManuallyRoll(final List<MyFrame> myFrames,
                                           final ImageView imageView, final Runnable onComplete,
                                           final int frameNumber, Boolean isNewStart) {
        final MyFrame thisFrame = myFrames.get(frameNumber);
        ...
        }else
        {
            ...
            if (frameNumber == 0) {
                previousFrame = myFrames.get(myFrames.size() - 1);
            } else {
                ...
                if (imageView.getDrawable() == thisFrame.drawable) {
                    if (true) {
                        ...
                        if (nextFrame.isReady) {
                            // Animate next frame
                            animateRawManuallyRoll(myFrames, imageView, onComplete,
                                    (frameNumber + 1)%myFrames.size(), false);
                        } else {
                            nextFrame.isReady = true;
                        }
                    }
